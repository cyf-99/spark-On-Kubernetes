...
def login():
    if request.method == 'POST':
        ...
        with open("/home/cyf/flask_spark/shell/password.csv", "a+") as fp:
            fp.writelines(user_name + "," + request.form["password"] + "\n")
        client.mkdirs(path + user_name, permission="777")
        client.mkdirs(path + user_name + "/code", permission="777")
        client.mkdirs(path + user_name + "/data", permission="777")
        client.mkdirs(path + user_name + "/result", permission="777")
...
def operation():
    if request.method == 'POST':
        ...
        if 'back_operation' in request.form:
            ...
            print(int(result['user_name']) ,list(password.index) or \
                    password.loc[int(result['user_name'])]['password'] , int(result['password']))
            if int(result['user_name']) not in list(password.index) or \
                    password.loc[int(result['user_name'])]['password'] != int(result['password']):
                return render_template('login.html')
...
def run():
    if request.method == 'POST':
        ...
        if '运行' in request.form:
            ...
            flag, result, status = get_result(session['user_name'], code_name)
